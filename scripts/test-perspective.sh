...
if (( ${#modes[@]} == 0 )); then
    echo "error: PERSPECTIVE_MODES is empty" >&2
    exit 2
fi
for mode in "${modes[@]}"; do
    case "$mode" in
        top-narrow-4|bottom-narrow-4) ;;
        *) echo "error: unsupported positive perspective mode: $mode" >&2; exit 2 ;;
    esac
done
...
for image in "${images[@]}"; do
    ...
    if ! "$PIXSEAL" embed -in "$image" -out "$marked" -key "$TEST_KEY" \
        -message "$TEST_MESSAGE_ROBUST" -profile robust >/dev/null; then
        echo "  ERROR embed            could not create robust hidden payload"
        ((errors += 1))
        ((cases += ${#modes[@]}))
        continue
    fi
    ...
    for mode in "${modes[@]}"; do
        case "$mode" in
            top-narrow-4)
                pts="0,0 $x1,0 $maxx,0 $x2,0 0,$maxy 0,$maxy $maxx,$maxy $maxx,$maxy" ;;
            bottom-narrow-4)
                pts="0,0 0,0 $maxx,0 $maxx,0 0,$maxy $x1,$maxy $maxx,$maxy $x2,$maxy" ;;
        esac
        transformed="$tmp_dir/$mode-$index.png"
        ((cases += 1))
        if ! "${image_tool[@]}" "$marked" -alpha off -virtual-pixel white \
            -define "distort:viewport=${width}x${height}+0+0" \
            -distort Perspective "$pts" "$transformed" >/dev/null 2>&1; then
            printf '    ERROR %-18s ImageMagick transformation failed\n' "$mode"
            ((errors += 1))
            continue
        fi

        output="$(timeout --foreground "${EXTRACT_TIMEOUT}s" "$PIXSEAL" extract \
            -in "$transformed" -key "$TEST_KEY" 2>&1)"
        status=$?
        if (( status == 0 )); then
            extracted="${output%%$'\n'*}"
            correction="$(printf '%s\n' "$output" | sed -n 's/^perspective-correction: //p' | head -n 1)"
            if [[ "$extracted" == "$TEST_MESSAGE_ROBUST" && "$correction" == "$mode" ]]; then
...
if (( cases == 0 && skipped == 0 )); then
    echo "error: perspective suite executed zero cases" >&2
    exit 2
fi
